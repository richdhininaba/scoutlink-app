...
module.exports = async function handler(req, res) {
  ...
  if (contentPath) {
    ...
    routes = Object.keys(store.pages || {}).filter(route => !route.includes('{'));
  }
  routes.push('/award-ceremonies');

  const [posts, jobs] = await Promise.all([
    fetchSupabaseRows('stratex_learning_posts', 'select=slug&status=eq.published&index_when_published=eq.true'),
    fetchSupabaseRows('job_posts', 'select=slug,status&status=in.(published,open,live)')
  ]);
  posts.forEach(post => { if (post && post.slug) routes.push(`/learning-centre/${encodeURIComponent(post.slug)}`); });
  jobs.forEach(job => { if (job && job.slug) routes.push(`/careers/${encodeURIComponent(job.slug)}`); });
