...
(function(){
  ...
  function fieldHeader(title,sub,right,left){if(window.CoachV2&&window.CoachV2.setFieldHeader){var a=[title];if(arguments.length>1)a.push(sub==null?'':sub);if(arguments.length>2)a.push(right==null?'':right);if(arguments.length>3)a.push(left==null?'':left);window.CoachV2.setFieldHeader.apply(window.CoachV2,a);}return'';}
  ...
  function bindPlayers(){
    ...
    document.querySelectorAll('[data-phone-player-view]').forEach(function(b){b.onclick=function(){var v=b.dataset.phonePlayerView;if(v==='work'){S.filters.evidence='work';S.filters.interest='';}else if(v==='interest'){S.filters.interest='yes';S.filters.evidence='';}else{S.filters.evidence='';S.filters.interest='';}renderPlayers();};});
  }
  ...
  function fixturePhoneDetail(f){
    ...
    return fieldHeader('vs '+(f.opponent||'Opponent'),fmtDate(f.fixture_date,f.fixture_time),'<button class="ic" aria-label="Fixture actions">⋯</button>','<button class="bk" id="fixturePhoneBack">‹</button>')+
      '<div class="stack">'+
        '<div class="card"><div class="mobile-card-title" style="margin-bottom:8px">Fixture detail</div><div class="kv"><span>Home / away</span><b>'+esc(f.home_or_away||'—')+'</b></div><div class="kv"><span>Format</span><b>'+esc((f.format||'—')+(String(f.format||'').indexOf('v')<0&&f.format?'v'+f.format:''))+'</b></div><div class="kv"><span>Venue</span><b>'+esc(f.venue||'—')+'</b></div>'+(address?'<div class="kv"><span>Address</span><b style="text-align:right">'+esc(address)+'</b></div><a class="bt blk" style="margin-top:10px" target="_blank" rel="noopener" href="https://www.google.com/maps/search/?api=1&query='+encodeURIComponent(address)+'">Directions</a>':'')+'</div>'+
  ...
  function fixturesPhone(){
    ...
    return fieldHeader('Fixtures',fixtures.length+' this season','<button class="icb" id="phoneAddFixture" aria-label="Add fixture">+</button>')+
  ...
  function videoProvider(url){try{var u=new URL(url,location.origin),h=u.hostname.toLowerCase();if(h==='youtu.be'||h.endsWith('youtube.com'))return'YouTube';if(h.endsWith('vimeo.com'))return'Vimeo';if(h.endsWith('drive.google.com'))return'Google Drive';if(h.endsWith('dropbox.com')||h.endsWith('dropboxusercontent.com'))return'Dropbox';if(h.indexOf('veo')>=0)return'Veo';if(h.indexOf('wyscout')>=0)return'Wyscout';if(h.indexOf('tonsser')>=0)return'Tonsser';return'External video';}catch(_){return'Video';}}
  function videoEmbedUrl(url){
    try{
      var u=new URL(url,location.origin),h=u.hostname.toLowerCase(),m,id;
      if(h==='youtu.be'){id=u.pathname.split('/').filter(Boolean)[0];return id?'https://www.youtube-nocookie.com/embed/'+encodeURIComponent(id):'';}
      if(h.endsWith('youtube.com')){id=u.searchParams.get('v');if(!id){m=u.pathname.match(/\/(?:embed|shorts)\/([^/?#]+)/);id=m&&m[1];}return id?'https://www.youtube-nocookie.com/embed/'+encodeURIComponent(id):'';}
      if(h.endsWith('vimeo.com')){m=u.pathname.match(/\/(?:video\/)?(\d+)/);return m?'https://player.vimeo.com/video/'+m[1]:'';}
      if(h.endsWith('drive.google.com')){m=u.pathname.match(/\/file\/d\/([^/]+)/);if(m)return'https://drive.google.com/file/d/'+encodeURIComponent(m[1])+'/preview';id=u.searchParams.get('id');if(id)return'https://drive.google.com/file/d/'+encodeURIComponent(id)+'/preview';}
      return'';
    }catch(_){return'';}
  }
  function videoPreviewHtml(v){
    var url=videoUrl(v),embed=videoEmbedUrl(url),provider=videoProvider(url),direct=!externalVideo(v)||/\.(mp4|webm|ogg|mov)(?:$|[?#])/i.test(url);
    if(!url)return'<div class="video-preview-empty">Preview is not available for this video.</div>';
    if(direct)return'<div class="video-preview-shell"><video controls playsinline preload="metadata" src="'+esc(url)+'"></video></div>';
    if(embed)return'<div class="video-preview-shell"><iframe src="'+esc(embed)+'" title="'+esc(provider)+' video preview" allow="autoplay; encrypted-media; picture-in-picture; fullscreen" allowfullscreen referrerpolicy="strict-origin-when-cross-origin"></iframe></div><div class="video-preview-foot"><span class="mut">Playing from '+esc(provider)+'. Provider permissions still apply.</span><a class="btn sm" href="'+esc(url)+'" target="_blank" rel="noopener noreferrer">Open on provider</a></div>';
    return'<div class="video-preview-empty"><b>'+esc(provider)+' does not expose a reliable embeddable player for this link.</b><span>ScoutLink can keep the link, but playback must open on the provider.</span><a class="btn p" href="'+esc(url)+'" target="_blank" rel="noopener noreferrer">Open '+esc(provider)+'</a></div>';
  }
  ...
  function videoPhone(){
    ...
    return fieldHeader('Video Reels',pending.length+' awaiting review','<button class="icb" id="phoneUploadVideo" aria-label="Add video">+</button>')+
      ...
      '<div class="card phone-video-coverage" style="margin-top:10px"><div class="card-b" style="text-align:center"><svg class="phone-ring-svg" viewBox="0 0 120 120" width="88" height="88" aria-label="Approved video coverage '+coverage+' percent"><circle cx="60" cy="60" r="46" fill="none" stroke="var(--canvas2)" stroke-width="11"></circle><circle cx="60" cy="60" r="46" fill="none" stroke="var(--blue)" stroke-width="11" stroke-dasharray="'+(coverage/100*289).toFixed(1)+' 289" transform="rotate(-90 60 60)"></circle><text x="60" y="58" text-anchor="middle" font-size="23" font-weight="700" fill="var(--ink)">'+coverage+'%</text><text x="60" y="76" text-anchor="middle" font-size="9" fill="var(--ink3)">approved</text></svg><div class="mobile-card-title" style="justify-content:center;margin-top:6px">Approved video coverage</div><div class="mut" style="margin-top:4px">'+without.length+' players still need approved video evidence.</div></div></div>'+
      ...
      '<div class="pcap">No approved video <span>'+without.length+'</span></div><div class="card">'+without.slice(0,10).map(function(p){return'<div class="rowline"><span class="who"><b>'+esc(name(p))+'</b><span>'+esc(position(p)+' · '+(p.age_group||''))+'</span></span><button class="bt sm" data-generate-link-player="'+esc(p.id)+'">Generate link</button></div>';}).join('')+(without.length?'':'<div class="card-b mut">Every player has approved video.</div>')+'</div>'+
      '<button class="bt spend blk" id="phoneGenerateLink" style="margin-top:10px">Generate upload links</button>'+
      '<div class="pcap">Approved <span>'+approved.length+' clips</span></div><div class="card">'+(approved.length?approved.slice(0,10).map(function(v){var p=playerById(v.player_id);return'<div class="rowline"><span class="icn g">▶</span><span class="who"><b>'+esc(v.title||name(p)+' · highlights')+'</b><span>'+esc(name(p)+' · '+fmtDate(v.created_at)+' · '+videoProvider(videoUrl(v)))+'</span></span><button class="bt sm" data-video-preview="'+esc(v.id)+'">Preview</button></div>';}).join(''):'<div class="card-b mut">No approved videos yet.</div>')+'</div>';
  }
  function videoById(id){return rows(S.overview||{},['videos']).find(function(v){return String(v.id)===String(id)});}
  function reviewVideo(id){
    var v=videoById(id);if(!v)return;
    window.CoachV2.openDrawer({title:'Review video',html:'<div class="video-review-meta"><b>'+esc(v.title||'Video')+'</b><span class="mut">'+esc(name(playerById(v.player_id))+' · '+(v.category||v.video_type||'Highlight')+' · '+videoProvider(videoUrl(v)))+'</span></div>'+videoPreviewHtml(v),footer:'<button class="btn dgr" data-video-moderate="'+esc(v.id)+'" data-decision="rejected">Reject</button><button class="btn p" data-video-moderate="'+esc(v.id)+'" data-decision="approved">Approve</button>'});setTimeout(bindVideos,0);
  }
