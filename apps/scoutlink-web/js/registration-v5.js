...
(function () {
  ...
  var PLAN_DATA = [
    {
      ...
      annual:'£599/year',
      monthly:'£69/month',
      perSeat:'£599 / seat / year',
      features:[
        'U7 to U16 player database search',
        ...
        '20 exports/year',
        '60 predictions/year',
        '30 coach-mediated interest requests/year'
      ]
    },
    {
      name:'Plus',
      ...
      annual:'£1,999/year',
      monthly:'£219/month',
      perSeat:'£400 / seat / year',
      popular:true,
      features:[
        'Everything in Core',
        'shared team usage',
        'stronger team workflow',
        'more player dossier exports',
        'more prediction usage.',
        '100 exports/year',
        '300 predictions/year',
        '120 coach-mediated interest requests/year'
      ]
    },
    {
      name:'Elite',
      ...
      features:[
        ...
        'higher usage limits.',
        '300 exports/year',
        '900 predictions/year',
        '300 coach-mediated interest requests/year'
      ]
    },
    {
      name:'Enterprise',
  ...
  function queryPlan() {
    ...
    try {
      ...
      return PLAN_DATA.some(function (plan) { return plan.name === requested; }) ? requested : '';
    } catch (_) {
      return '';
    }
  }
  ...
  function load() {
    try {
      ...
      if (saved && saved.values) {
        state.step = Math.max(1, Math.min(MAX_STEP, Number(saved.step) || 1));
        state.values = Object.assign(state.values, saved.values);
        state.checks = saved.checks || {};
      }
    } catch (_) {}

    var requestedPlan = queryPlan();
    if (requestedPlan) {
      state.values.preferredScoutPlan = requestedPlan;
      state.step = 1;
    }
  }
  ...
  function brandMark(width, height) {
    return '<span class="sl-logo-mark white logo" style="width:' + width + 'px;height:' + height + 'px" aria-hidden="true"></span>';
  }

  function shell(content) {
    var tag = role === 'coach' ? 'Coach registration · Free' : 'Scout access application';
    return '<div class="site registration-page">' +
      '<header class="gnav slim">' +
        '<div class="gnav-left">' +
          '<a class="sl-mark" href="/login" aria-label="ScoutLink sign in">' +
            brandMark(128, 41) + '<small>by Stratex Analytics</small>' +
          '</a>' +
          '<span class="tag">' + esc(tag) + '</span>' +
        '</div>' +
        '<div class="gnav-cta">' +
          '<a class="sl-btn ghostw sm" href="/demo">See demo</a>' +
          '<a class="signin" href="/login">Sign in</a>' +
        '</div>' +
      '</header>' +
      content +
      '<footer class="sl-foot">' +
        '<a class="sl-mark" href="/login" aria-label="ScoutLink">' + brandMark(100, 32) + '</a>' +
        '<div class="links">' +
          '<a href="/demo">Demo</a>' +
          '<a href="https://www.stratexanalytics.co.uk/trust">Safeguarding</a>' +
          '<a href="https://www.stratexanalytics.co.uk/privacy-policy">Privacy</a>' +
        '</div>' +
        '<span>A Stratex Analytics product</span>' +
      '</footer>' +
    '</div>';
  }
  ...
  function stepRail() {
    var labels = role === 'coach'
      ...
      : ['Choose a plan','Who you are','Organisation','Declarations'];

    return '<div class="steprail">' + labels.map(function (label, index) {
      var number = index + 1;
      var cls = number === state.step ? 'cur' : number < state.step ? 'done' : '';
  ...
  function field(label, name, type, options, required, help) {
    ...
    if (type === 'select') {
      control = '<select class="sl-in" name="' + esc(name) + '"' + req + '>' +
      ...
      control = '<input class="sl-in" name="' + esc(name) + '" type="' + esc(type || 'text') +
        '" value="' + esc(value) + '"' + req + '>';
    }

    return '<div class="sl-field">' +
      '<label class="sl-lab">' + esc(label) + (required === false ? '' : ' <em>Required</em>') + '</label>' +
      control +
      (help ? '<small class="sl-help">' + esc(help) + '</small>' : '') +
    '</div>';
  }
  ...
  function two(a, b) {
    return '<div class="sl-grid2">' + a + b + '</div>';
  }
  ...
  function scoutStageMap() {
    return '<section style="margin-bottom:32px">' +
      '<span class="sl-eb">Before you start</span>' +
      '<h2 class="sl-h" style="font-size:26px">Four stages to player search.</h2>' +
  ...
  function planCards() {
    return '<label class="sl-lab" style="margin-bottom:14px;display:block">' +
      'Preferred ScoutLink plan <em>Preference only</em>' +
    '</label>' +
    '<div class="plancards">' +
      PLAN_DATA.map(function (plan) {
        var selected = state.values.preferredScoutPlan === plan.name;
        var classes = ['plancard'];
        if (plan.popular) classes.push('popular');
        if (selected) classes.push('on');

        return '<button class="' + classes.join(' ') + '" type="button" data-plan="' + esc(plan.name) +
          '" aria-pressed="' + (selected ? 'true' : 'false') + '">' +
          (plan.popular ? '<span class="badge-pop">Most teams pick this</span>' : '') +
          '<span class="seats-line">' + esc(plan.seat) + '</span>' +
          '<h4>' + esc(plan.name) + '</h4>' +
          '<div class="price-big">' + esc(plan.annual) + '</div>' +
          '<div class="price-monthly">' + esc(plan.monthly) + '</div>' +
          (plan.perSeat ? '<span class="per-seat-chip">' + esc(plan.perSeat) + '</span>' : '') +
          '<ul>' + plan.features.map(function (feature) {
            return '<li>' + esc(feature) + '</li>';
          }).join('') + '</ul>' +
          '<div class="radio-row"><span class="radio"></span><span>Select ' + esc(plan.name) + '</span></div>' +
        '</button>';
      }).join('') +
    '</div>';
  }
  ...
  function check(name, title, copy) {
    return '<label class="sl-check">' +
      '<input type="checkbox" name="' + esc(name) + '"' + (state.checks[name] ? ' checked' : '') + ' required>' +
      '<span><b>' + esc(title) + '</b><small>' + esc(copy) + '</small></span>' +
    '</label>';
  }
  ...
  function scoutDeclarationsStep() {
    ...
    return check(
      ...
      'verification',
      'I will complete identity and safeguarding verification',
      'The documents are requested through a separate secure email link.'
    ) + check(
      'responsible',
      'I accept the terms and responsible-access commitments',
      'Access can be reviewed or withdrawn where necessary to protect players.'
    ) +
    '<div class="reg-summary">' +
      summaryRow('Applicant', (state.values.fullName || '') + ' · ' + (state.values.currentScoutingRole || '')) +
      summaryRow('Organisation', (state.values.scoutClub || '') + ' · ' + (state.values.primaryScoutingRegion || '')) +
      summaryRow('Plan preference', selectedPlan.name + ' · ' + selectedPlan.seat + ' · ' + selectedPlan.annual + ' · no payment taken') +
    '</div>';
  }
  ...
  function titleForStep() {
    ...
    return [
      ['Choose a plan.','Compare the current plan price, seats and benefits before anything else. Nothing is charged until after approval.'],
      ['Who you are.','Professional details that can be matched to your scouting role. Your password is created after approval.'],
      ['Organisation.','This describes the organisation around your search and who else will use it.'],
  ...
  function formBody() {
    if (role === 'coach') {
      if (state.step === 1) return coachStep1();
      if (state.step === 2) return coachStep2();
      return coachDeclarationsStep();
    }

    if (state.step === 1) return planCards();
    if (state.step === 2) return scoutPersonalStep();
    if (state.step === 3) return scoutOrganisationStep();
    return scoutDeclarationsStep();
  }

  function sideCopy() {
    if (role === 'coach') {
      return '<aside class="reg-side">' +
        '<div class="sl-aside"><b>Free means free</b><p>No card details at any step and no trial period. Coach workspaces are free for grassroots football.</p></div>' +
        '<div class="sl-aside"><b>Saved as you go</b><p>Progress is stored locally between steps on this device.</p></div>' +
      '</aside>';
    }

    return '<aside class="reg-side">' +
      '<div class="sl-aside"><b>Reviewed access</b><p>This is youth football. Identity, safeguarding information and role context are reviewed before player-search access.</p></div>' +
      '<div class="sl-aside"><b>Clear pricing before approval</b><p>Annual and monthly pricing, seat allowance and key benefits are shown when you choose a plan. No payment is taken on this application.</p></div>' +
    '</aside>';
  }

  function scoutPlanNotes() {
    return '<div class="sl-grid2" style="margin-top:22px">' +
      '<div class="sl-aside"><b>Reviewed access</b><p>This is youth football. Identity, safeguarding information and role context are reviewed before player-search access.</p></div>' +
      '<div class="sl-aside"><b>Clear pricing before approval</b><p>Annual and monthly pricing, seat allowance and key benefits are shown when you choose a plan. No payment is taken on this application.</p></div>' +
    '</div>';
  }
  ...
  function actions() {
    var finalStep = state.step === MAX_STEP;
    var nextLabel = finalStep ? (role === 'coach' ? 'Create workspace' : 'Submit application') : 'Continue';

    return '<div class="reg-actions">' +
      (state.step > 1
        ? '<button class="sl-btn line" type="button" data-back>Back</button>'
        : '<a class="sl-btn line center" href="/register">Back</a>') +
      '<button class="sl-btn pitch" id="regNext" type="submit">' +
        '<span>' + esc(nextLabel) + '</span><i class="sl-spinner" aria-hidden="true"></i>' +
      '</button>' +
    '</div>';
  }
  ...
  function render() {
    ...
    var error = '<div class="sl-message err" id="regError" role="alert" aria-live="assertive"></div>';
    var form = '<form id="registrationForm" novalidate>' +
      formBody() +
      (role === 'scout' && state.step === 1 ? scoutPlanNotes() : '') +
      actions() +
    '</form>';

    if (role === 'scout' && state.step === 1) {
      root.innerHTML = shell(
        '<main class="register-shell" style="max-width:1080px">' +
          '<section class="reg-panel">' +
            scoutStageMap() +
            stepRail() +
            '<h1>' + esc(heading[0]) + '</h1>' +
            '<p class="lead">' + esc(heading[1]) + '</p>' +
            error +
            form +
          '</section>' +
        '</main>'
  ...
  function renderSuccess() {
    ...
    root.innerHTML = shell(
      '<main class="registration-wrap single">' +
        '<section class="reg-panel reg-success">' +
          ...
          '<div class="sl-aside">' +
            '<b>' + (isCoach ? 'What happens next' : 'Stage two comes next') + '</b>' +
            '<p>' +
              (isCoach
                ? 'Watch your email for the account-completion step. No password was created on this form.'
          ...
          '<div class="reg-actions">' +
            '<a class="sl-btn line center" href="/demo">Open demo</a>' +
            '<a class="sl-btn pitch center" href="/login">Go to sign in</a>' +
          '</div>' +
        '</section>' +
      '</main>'
    );
  ...
  function bind() {
    ...
    document.querySelectorAll('[data-plan]').forEach(function (button) {
      button.addEventListener('click', function () {
        state.values.preferredScoutPlan = button.getAttribute('data-plan');
        save();
        render();
      });
    });
