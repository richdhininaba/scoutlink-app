...
(function () {
  ...
  function metric(label,value,big){return'<div><div class="lbl">'+esc(label)+'</div><div class="'+(big?'num':'')+'" style="font-size:'+(big?'23':'14')+'px;font-weight:700;margin-top:'+(big?'0':'6')+'px">'+esc(value)+'</div></div>';}
  ...
  function phoneHero(p){
    ...
    return
      '<div class="card hero"><div class="flex" style="gap:9px"><span class="avm" style="width:40px;height:40px;font-size:12px">'+esc(initials(name(p)))+'</span><span class="who" style="flex:1"><b style="font-size:15px">'+esc(name(p))+'</b><span>'+esc(positionLabel(pos(p))+' · '+team(p))+'</span></span></div><div class="pkpi" style="margin-top:10px"><div class="kpi"><div class="k">Overall</div><div class="v">'+esc(overall(p))+'</div></div><div class="kpi"><div class="k">Readiness</div><div class="v">'+readiness(p)+'%</div></div></div><div class="flex" style="gap:6px;margin-top:8px">'+tags(p)+'</div></div>'+
      '<div class="seg" style="margin-top:10px">'+[['overview','Overview'],['attributes','Attributes '+c.total],['facts','Facts '+matches().length],['video','Video '+videos().length]].map(function(x){return'<a href="#" data-profile-tab="'+x[0]+'" class="'+(S.tab===x[0]?'on':'')+'">'+x[1]+'</a>';}).join('')+'</div>';
  }
  function phone(p){
    var c=counts(p),r=ratings(p),sections=applicable(p);
    if(S.tab==='attributes')return phoneHero(p)+'<div class="pcap">Applicable set <span>'+c.rated+'/'+c.total+' rated</span></div>'+sections.map(function(sec){return'<div class="card rule" style="margin-bottom:10px"><div class="ck" style="margin-bottom:6px">'+esc(sec.label)+'</div>'+sec.rows.map(function(row){var v=n(r[row[0]],NaN);return'<div class="rowline"><span class="who"><b>'+esc(row[1])+'</b><span>'+(Number.isFinite(v)?'Profile assessment':'Not observed')+'</span></span><span class="ratem '+(Number.isFinite(v)?'set':'')+'">'+(Number.isFinite(v)?v:'—')+'</span></div>';}).join('')+'</div>';}).join('')+'<button class="bt spend blk" data-profile-edit>Edit assessment</button>';
    if(S.tab==='facts')return phoneHero(p)+'<div class="pcap">Match Facts <span>'+matches().length+'</span></div><div class="card">'+matches().slice(0,8).map(function(m){return'<div class="rowline"><span class="who"><b>vs '+esc(m.opponent||m.opponent_name||'Opponent')+'</b><span>'+esc(formatDate(m.match_date))+' · '+esc(m.position_played||pos(p))+'</span></span><span class="ratem set">'+score(m)+'</span></div>';}).join('')+'</div><a class="bt spend blk" style="margin-top:10px" href="'+esc(route('/coach/match-facts?playerId='+encodeURIComponent(p.id||'')))+'">Record Match Facts</a>';
    if(S.tab==='video')return phoneHero(p)+'<div class="pcap">Video <span>'+approvedVideos().length+' approved · '+pendingVideos().length+' pending</span></div><div class="card">'+videoRows(p)+'</div><div class="g" style="grid-template-columns:1fr 1fr;margin-top:10px"><button class="bt" data-profile-upload>Upload</button><button class="bt spend" data-profile-upload-link>Generate link</button></div>';
    return phoneHero(p)+'<div class="pcap">Readiness <span>'+readiness(p)+'%</span></div><div class="card rule"><div class="rowline"><span class="who"><b>Attributes</b><span>'+c.rated+' of '+c.total+' rated</span></span><span class="tag '+(c.missing?'a':'g')+'">'+(c.missing?c.missing+' missing':'Complete')+'</span></div><div class="rowline"><span class="who"><b>Match Facts</b><span>'+matches().length+' records</span></span><span class="tag '+(matches().length?'g':'a')+'">'+(matches().length?'Added':'Add')+'</span></div><div class="rowline"><span class="who"><b>Approved video</b><span>'+approvedVideos().length+' clips</span></span><span class="tag '+(approvedVideos().length?'g':'a')+'">'+(approvedVideos().length?'Added':'Add')+'</span></div></div>'+
      '<div class="pcap">Development trend <span>last '+Math.min(9,matches().length)+'</span></div><div class="card"><div class="card-b">'+lineChart(matches().slice().reverse().map(function(m){return n(m.performance_score,NaN);}).filter(Number.isFinite).slice(-9))+'</div></div>'+
      '<div class="pcap">Recent Match Facts</div><div class="card">'+matches().slice(0,4).map(function(m){return'<div class="rowline"><span class="who"><b>vs '+esc(m.opponent||'Opponent')+'</b><span>'+esc(formatDate(m.match_date))+'</span></span><span class="ratem set">'+score(m)+'</span></div>';}).join('')+'</div>'+
  ...
  function uploadLink(){
    ...
    setTimeout(function(){var b=document.getElementById('makeProfileLink');if(b)b.onclick=function(){b.disabled=true;api('POST','/api/videos/upload-link',{playerId:p.id}).then(function(r){var d=r.data||r,url=d.uploadUrl||d.url||'';document.getElementById('profileLinkResult').innerHTML='<div class="linkbox">'+esc(url)+'</div><button class="btn sm" id="copyProfileLink">Copy link</button>';document.getElementById('copyProfileLink').onclick=function(){navigator.clipboard.writeText(url);window.CoachV2.showToast('Link copied.');};}).catch(function(e){document.getElementById('profileLinkResult').innerHTML='<div class="coach-route-message error">'+esc(e.message)+'</div>';}).finally(function(){b.disabled=false;});};},0);
  }
  function uploadFile(){
    var p=player();window.CoachV2.openSheet({title:'Upload video',html:'<form id="profileUploadForm"><div class="field"><label>Title</label><input class="in" name="title" required></div><div class="field"><label>Category</label><select class="in" name="category"><option>Highlight</option><option>Match</option><option>Training</option><option>Skills</option></select></div><div class="field"><label>Video file</label><input class="in" name="file" type="file" accept="video/*" required></div><div id="profileUploadMsg"></div><button class="btn p" type="submit">Upload for review</button></form>'});
    setTimeout(function(){var f=document.getElementById('profileUploadForm');if(f)f.onsubmit=function(e){e.preventDefault();var fd=new FormData(f),file=fd.get('file');if(!file||!file.size)return;var body=new FormData();body.append('file',file);body.append('title',fd.get('title')||file.name);body.append('category',fd.get('category')||'Highlight');body.append('playerId',p.id);fetch((window.API||'')+'/api/videos/upload',{method:'POST',headers:{Authorization:'Bearer '+((window.Auth&&window.Auth.token)||'')},body:body}).then(function(r){return r.json().then(function(d){if(!r.ok)throw new Error(d.error||'Upload failed');return d;});}).then(function(){window.CoachV2.closeAll();window.CoachV2.showToast('Video uploaded for review.');location.reload();}).catch(function(err){document.getElementById('profileUploadMsg').innerHTML='<div class="coach-route-message error">'+esc(err.message)+'</div>';});};},0);
  }
  function reviewVideo(id){
    var v=videos().find(function(x){return String(x.id)===String(id);});if(!v)return;
    var url=v.signed_url||v.video_url||v.url||'';
    window.CoachV2.openSheet({title:'Review video',html:'<div class="card"><div class="card-b"><b>'+esc(v.title||'Video')+'</b><div class="mut">'+esc(v.category||v.video_type||'Highlight')+'</div>'+(url?'<video controls style="width:100%;margin-top:10px" src="'+esc(url)+'"></video>':'<div class="mut" style="margin-top:10px">Video preview unavailable.</div>')+'</div></div>',footer:'<button class="btn dgr" data-video-decision="rejected">Reject</button><button class="btn p" data-video-decision="approved">Approve</button>'});
