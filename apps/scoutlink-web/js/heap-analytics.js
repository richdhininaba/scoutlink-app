...
(function () {
  ...
  function isProfileRoute() {
    var path = routePath();
    return path.indexOf('/player/profile') === 0 ||
      path.indexOf('/public-demo/coach/player/profile') === 0 ||
      path.indexOf('/public-demo/scout/player/profile') === 0 ||
      path.indexOf('player-profile') > -1;
  }

  function declaredScoutRoute() {
    if (!document.body) return '';
    var declared = document.body.getAttribute('data-scout-route');
    var hasScoutBody = document.body.classList.contains('scout-experience-body');
    return declared && hasScoutBody ? String(declared).toLowerCase() : '';
  }

  function isExperienceRoute() {
    var path = routePath();
    var role = experienceRole();
    return /\/(coach|scout|player|admin|stratex)(\/|$)/.test(path) ||
      path.indexOf('/company/admin') === 0 ||
      /(?:coach|scout|player|stratex)-/.test(path) ||
  ...
  function isCoachRuntimeRoute() {
    var path = routePath();
    var role = experienceRole();
    var demoRouteRole = publicDemoRouteRole();

    return demoRouteRole === 'coach' ||
  ...
  function scoutRouteKey() {
    var declared = declaredScoutRoute();
    if (declared) return declared;

    var path = routePath();
    if (path.indexOf('compare-players') >= 0) return 'compare';
    if (path.indexOf('/predictions') >= 0 || path.indexOf('scout-predictions') >= 0) return 'predictions';
    if (path.indexOf('/rankings') >= 0 || path.indexOf('scout-rankings') >= 0) return 'rankings';
    if (isProfileRoute()) return 'profile';
    if (path.indexOf('/pipeline') >= 0) return 'pipeline';
    if (path.indexOf('/setup') >= 0 || path.indexOf('/onboarding') >= 0) return 'setup';
    if (path.indexOf('/player-search') >= 0) return 'search';
    ...
    if (path.indexOf('/usage-requests') >= 0) return 'usage';
    return '';
  }

  function shouldLoadScoutCore() {
    var route = scoutRouteKey();
    return ['compare','predictions','rankings'].indexOf(route) < 0;
  }
  ...
  function isScoutLoadingRoute() {
    ...
    if (declaredScoutRoute()) return true;
  ...
  function installScoutLoadingStatus() {
    ...
    function syncLoading() {
      ...
      try {
        computedVisibility = window.getComputedStyle(app).visibility;
      } catch (_) {}
  ...
  function loadAll() {
    loadScript(
      'publicDemoRoutingV2Script',
      'public-demo-routing-v2.js',
      '/js/public-demo-routing-v2.js'
    );

    loadScript(
      'dataMediaGuardV1Script',
      'data-media-guard-v1.js',
      '/js/data-media-guard-v1.js'
    );
    loadScript(
      'scoringV4ClientScript',
      'scoring-v4-client.js',
      '/js/scoring-v4-client.js'
    );
    loadScript(
      'playerInitialsV1Script',
      'player-initials-v1.js',
      '/js/player-initials-v1.js'
    );

    if (isExperienceRoute()) {
      loadScript(
        'experienceShellV7Script',
        'experience-shell-v1.js',
        '/js/experience-shell-v1.js'
      );
    }

    if (isCoachRuntimeRoute()) {
      loadStylesheet(
        'coachLayoutCoreV1Css',
    ...
    if (isScoutRuntimeRoute()) {
      /*
       * Must load before the page-specific repair modules so their requests and
       * query parameters use the final pre-deploy contracts from first run.
       */
      loadScript(
        'scoutPredeploySafetyV1Script',
        'scout-predeploy-safety-v1.js',
        '/js/scout-predeploy-safety-v1.js'
      );

      if (shouldLoadScoutCore()) {
        loadStylesheet(
        ...
        loadScript(
          ...
          '/js/scout-prediction-overlays-v2.js'
        );
        loadStylesheet(
        ...
        loadScript(
          ...
          '/js/scout-experience-core-v2.js'
        );
      }
      ...
      loadScript(
        ...
        'scout-scoring-v4.js',
        '/js/scout-scoring-v4.js'
      );
      loadScript(
        'scoutDashboardSearchV1Script',
        'scout-dashboard-search-v1.js',
        '/js/scout-dashboard-search-v1.js'
      );
      loadScript(
        'scoutProfileRankingV1Script',
        'scout-profile-ranking-v1.js',
        '/js/scout-profile-ranking-v1.js'
      );
      loadScript(
        'scoutComparePredictionsV1Script',
        'scout-compare-predictions-v1.js',
        '/js/scout-compare-predictions-v1.js'
      );
      loadScript(
        'scoutFixturesUsageV1Script',
        'scout-fixtures-usage-v1.js',
        '/js/scout-fixtures-usage-v1.js'
      );
      loadScript(
        'scoutChatNotificationsCleanupV1Script',
        'scout-chat-notifications-cleanup-v1.js',
        '/js/scout-chat-notifications-cleanup-v1.js'
      );
    }

    if (isDemo()) {
      loadScript(
        'demoScoringV4AuthoritativeScript',
        'demo-scoring-v4-authoritative.js',
        '/js/demo-scoring-v4-authoritative.js'
      );
      loadScript(
        'demoScoringV4Script',
        'demo-scoring-v4.js',
        '/js/demo-scoring-v4.js'
      );
    }

    /*
     * The deleted scout-profile-runtime-v1.js is intentionally not loaded.
     * Coach-only output fixes are also kept out of Scout and Player profiles.
     */
    if (isProfileRoute() && isCoachRuntimeRoute()) {
      loadScript(
        'coachProfileV4OutputFixesScript',
        'coach-profile-v4-output-fixes.js',
        '/js/coach-profile-v4-output-fixes.js'
      );
    }
  }
  ...
  function stableUserId(context) {
    ...
    return !candidate || UNSAFE_IDS[candidate.toLowerCase()]
      ? null
      : candidate;
  }
  ...
  function applyHeapContext() {
    ...
    try {
      ...
      window.heap.addUserProperties(compact({
        Role: role,
        AccountType:
          safe(pick(user, ['accountType','account_type'])) || role,
        SelectedExperience: selectedExperience(role),
        DemoMode: !!isDemo(),
        PublicDemo: !!isPublicDemo(),
        PublicDemoRole: safe((function () {
          try {
