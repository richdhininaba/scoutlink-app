...
(function(){
  ...
  function shouldRun(){
    ...
    return qs.get('tour') === '1' || qs.get('tour') === 'done' || !localStorage.getItem('sl_tour_seen_' + r);
  }
  function render(){
    ...
    var s = steps[index];
    existing.innerHTML = '<div style="width:min(560px,100%);background:#111827;border:1px solid #243447;border-radius:16px;padding:18px;box-shadow:0 24px 80px rgba(0,0,0,.45)"><div style="display:flex;justify-content:space-between;gap:12px;align-items:center;margin-bottom:8px"><span style="color:#00e676;font-weight:900;font-size:12px;text-transform:uppercase">ScoutLink tour '+(index+1)+'/'+steps.length+'</span><button class="btn btn-sm btn-ghost" onclick="window.finishProductTour()">Skip</button></div><h3 style="margin:0 0 8px;color:#fff">'+s[0]+'</h3><p style="color:#B0BEC5;line-height:1.55;margin:0 0 16px">'+s[1]+'</p><div style="display:grid;grid-template-columns:1fr 1fr;gap:10px"><button class="btn btn-outline" onclick="window.prevProductTour()" '+(index===0?'disabled':'')+'>Back</button><button class="btn btn-primary" onclick="window.nextProductTour()">'+(index===steps.length-1?'Finish':'Next')+'</button></div></div>';
  }
  window.prevProductTour = function(){ index = Math.max(0, index - 1); render(); };
  window.nextProductTour = function(){ var r = role(), steps = tours[r] || []; if (index >= steps.length - 1) return window.finishProductTour(); index++; render(); };
  window.finishProductTour = async function(){
    var r = role();
