...
(function scoutExperienceCoreV2Bootstrap() {
  if (window.__scoutExperienceCoreV2) return;
  window.__scoutExperienceCoreV2 = true;

  const VERSION = '20260731.4';
  const ROLE_TEXT =
  ...
  const CSS_URL =
    '/frontend/css/scout-experience-core-v2.css?v=' +
    VERSION;
  const STAGES = [
    ['watching', 'Watching'],
...
function canonicalPredictionType(value, result) {
  const candidates = [
    value?.prediction_type,
    value?.predictionType,
    value?.input_params?.predictionType,
    value?.inputParams?.predictionType,
  ...
  for (const candidate of candidates) {
    const normalised = String(candidate || '')
      .trim()
      .toLowerCase()
      .replace(/[_-]+/g, ' ');

    if (!normalised) continue;
    if (normalised.includes('roi') || normalised.includes('return on investment')) {
      return 'ROI Analysis';
    }
    if (normalised.includes('attribute') || normalised.includes('development')) {
      return 'Attribute Development';
    }
    if (normalised.includes('scenario') || normalised.includes('match prediction')) {
      return 'Match Scenario Prediction';
    }
    if (normalised.includes('position') || normalised.includes('role fit')) {
  ...
  const source = log || {};
  const result = source.result || source.predictionResult || source.data?.result || {};
  const explicitType = canonicalPredictionType(source, result);

  return {
    id:
      source.id ||
      source.logId ||
      source.predictionLogId ||
      source.prediction_log_id ||
      '',
    player_id:
      source.player_id ||
      source.playerId ||
  ...
  if (window.ScoutPredictionOverlayV2?.close) {
    window.ScoutPredictionOverlayV2.close();
  }

  const legacy = document.getElementById('slwfPredictionOverlay');
  if (legacy) {
    legacy.classList.remove('slwf-open');
  ...
  const player =
    log.player ||
    playerCache?.find((item) => String(item.id) === String(log.player_id)) ||
    fallbackPlayer ||
    {};

  if (!window.ScoutPredictionOverlayV2?.open) {
    toast('The detailed prediction overlay could not be loaded.', 'error');
    return;
  }

  window.ScoutPredictionOverlayV2.open(log, player, {
    ...
    onRunAnother: () => {
      ...
      const target =
        ...
        encodeURIComponent(player.id || log.player_id || '') +
        '&type=' +
        encodeURIComponent(log.prediction_type);
      if (pathName().includes('/scout/predictions')) {
        const shadow = exactShadow();
        const typeButton = Array.from(
          shadow?.querySelectorAll('.prediction-type') || []
        ).find((button) =>
          selectedTypeFromButton(button) === log.prediction_type
        );
        typeButton?.click();
        shadow?.querySelector('.prediction-analysis-panel')?.scrollIntoView({
          behavior: 'smooth',
          block: 'start'
        });
      } else {
...
function demoPredictionConfidence(player) {
  const appearances = num(player?.appearances);
  if (appearances >= 10) {
    return {
      score: 90,
      label: 'High confidence',
      note: 'Ten or more recorded appearances support this demo result.'
    };
  }
  if (appearances >= 5) {
    return {
      ...
      note: appearances + ' recorded appearances support this demo result.'
    };
  }
  if (appearances > 0) {
    return {
      ...
      note: 'Fewer than five recorded appearances support this demo result.'
    };
  }
  return {
    score: 30,
    label: 'Very low confidence',
    note: 'No recorded Match Facts support this demo result yet.'
  };
}

function demoRoleRows(player) {
  const overall = clamp(
    num(player?.overall_rating) <= 10
      ? num(player?.overall_rating) * 10
      : num(player?.overall_rating),
    0,
    100
  );
  const current = playerPosition(player);
  const alternatives = {
    ST: ['CF', 'SS', 'RW', 'LW', 'CAM'],
    CF: ['ST', 'SS', 'CAM', 'RW', 'LW'],
    RW: ['LW', 'ST', 'CF', 'CAM', 'RWB'],
    LW: ['RW', 'ST', 'CF', 'CAM', 'LWB'],
    CAM: ['CM', 'RW', 'LW', 'CF', 'B2B'],
    CM: ['B2B', 'CAM', 'CDM', 'RW', 'LW'],
    B2B: ['CM', 'CDM', 'CAM', 'RB', 'LB'],
    CDM: ['CM', 'CB', 'B2B', 'RB', 'LB'],
    CB: ['BPD', 'CDM', 'RB', 'LB', 'CM'],
    BPD: ['CB', 'CDM', 'CM', 'RB', 'LB'],
    RB: ['RWB', 'LB', 'CB', 'RW', 'CDM'],
  ...
  const roles = [current].concat(alternatives[current] || ['CM', 'CAM', 'ST', 'CB', 'RW']);
  return unique(roles).slice(0, 6).map((roleName, index) => ({
    role: roleName,
    score: Math.round(clamp(overall - index * 3 + (index === 1 ? 1 : 0), 45, 99)),
    note:
      index === 0
        ? 'Best current role'
        : index === 1
          ...
          : index <= 3
            ? 'Natural alternative'
            : 'Higher-friction option'
  }));
}

function demoProjectionValue(base, overall, year) {
  const ratingGrowth = 1 + Math.max(0, overall - 65) / 240;
  return Math.round(
    Math.max(25000, base || 60000) *
    ratingGrowth *
    (1 + year * 0.075)
  );
}

function demoAttributeEffects(player, focus) {
  const keys = [
  ...
  const focusMap = {
    'Balanced Growth': keys,
    'Technical Possession': ['passing', 'vision', 'composure', 'dribbling', 'crossing'],
    'Final Third Output': ['shooting', 'positioning', 'dribbling', 'crossing', 'composure'],
    'Defensive Intelligence': ['defending', 'tackling', 'positioning', 'composure', 'heading'],
    'Physical Dominance': ['strength', 'stamina', 'jumping', 'heading', 'pace'],
    'Goalkeeper Command': ['gk_diving', 'gk_reflexes', 'gk_handling', 'gk_positioning', 'gk_communication']
  };
  const priority = new Set(focusMap[focus] || keys);
  return keys.map((key, index) => {
    const current10 = clamp(
      num(player?.[key], 5) > 10
        ? num(player?.[key], 50) / 10
        : num(player?.[key], 5),
      1,
      10
    );
    let delta = priority.has(key)
      ? 1.1 + (index % 3) * 0.25
      : 0.15;
    if (
      focus === 'Technical Possession' &&
      ['heading', 'jumping', 'pace', 'strength'].includes(key)
    ) {
      delta = ['heading', 'jumping'].includes(key) ? -0.4 : -0.2;
    }
    const final10 = clamp(current10 + delta, 1, 10);
    return {
      key,
      attribute: titleCase(key),
      currentScore: Math.round(current10 * 10),
      projectedScore: Math.round(final10 * 10),
      deltaFiveYear: Math.round(delta * 10) / 10,
      reason:
        delta < 0
          ? 'Trade-off requiring protected training load.'
          : priority.has(key)
            ? 'Priority gain from ' + focus + '.'
            : 'Maintained alongside the primary development focus.'
    };
  });
}

function runDemoPrediction(player, predictionType, inputParams) {
  const state =
    typeof getDemoState === 'function'
      ? getDemoState()
      : {};
  const type = canonicalPredictionType(predictionType);
  ...
  const currentValue = Math.max(0, Math.round(num(player?.transfer_value)));
  const confidence = demoPredictionConfidence(player);
  const roles = demoRoleRows(player);
  const currentRole = roles[0] || {
    role: playerPosition(player),
    score: overall
  };
  const futureRole = roles[1] || currentRole;
  let result;

  if (type === 'Attribute Development') {
    const focus = inputParams?.focus || 'Balanced Growth';
    const effects = demoAttributeEffects(player, focus);
    const seasons = Array.from({ length: 5 }, (_, index) => {
      const year = index + 1;
      const projectedOverall = Math.round(
        clamp(overall + year * 1.15, 0, 99)
      );
      const transferValue = demoProjectionValue(
        currentValue,
        projectedOverall,
        year
      );
      return {
        ...
        transferValue,
        transferValueFormatted: formatMoney(transferValue),
        rankingImpact:
          projectedOverall >= 85
            ? 'Elite academy watchlist range'
            : projectedOverall >= 75
              ? 'Strong regional academy range'
              : 'Developing pathway'
      };
    });
    const finalEffects = effects.filter((effect) => effect.deltaFiveYear > 0.5);
    const generated = {
      creativity: Math.round(average([
        demoAttr(player, 'passing'),
        demoAttr(player, 'vision'),
        demoAttr(player, 'dribbling')
      ])),
      ball_progression: Math.round(average([
        demoAttr(player, 'dribbling'),
        demoAttr(player, 'passing'),
        demoAttr(player, 'pace')
      ])),
      pressure_resistance: Math.round(average([
        demoAttr(player, 'composure'),
        demoAttr(player, 'dribbling'),
        demoAttr(player, 'strength')
      ])),
      attacking_threat: Math.round(average([
        demoAttr(player, 'shooting'),
        demoAttr(player, 'positioning'),
        demoAttr(player, 'pace')
      ])),
      defensive_reliability: Math.round(average([
        demoAttr(player, 'defending'),
        demoAttr(player, 'tackling'),
        demoAttr(player, 'positioning')
      ]))
    };
    result = {
      ...
      currentTransferValue: {
        value: currentValue,
        formatted: formatMoney(currentValue)
      },
      confidence,
      seasons,
      attributeEffects: effects,
      attributeEffectsByKey: Object.fromEntries(
        effects.map((effect) => [effect.key, effect])
      ),
      generatedAttributes: {
        5: generated
      },
      tradeOffs: effects.filter((effect) => effect.deltaFiveYear < 0),
      summary:
        finalEffects.length
          ? finalEffects.slice(0, 3).map((effect) => effect.attribute).join(', ') +
            ' show the clearest projected gains.'
          : 'The selected development plan produces a balanced five-year trajectory.',
      paragraphs: [
        'The selected focus was ' + focus + '. Every Coach-rated attribute remains visible so gains and trade-offs can be reviewed together.',
        'The year-by-year projection uses the player’s current rating, age band, evidence confidence and estimated value.'
      ],
      disclaimer:
        'Demo development projection. This is a deterministic decision-support estimate, not a guarantee.'
    };
  } else if (type === 'ROI Analysis') {
    const acquisitionCost =
      num(inputParams?.acquisitionCost) ||
      Math.round(currentValue * 0.18);
    const annualDevelopmentCost =
      num(inputParams?.annualDevelopmentCost) ||
      2500;
    const scoutingCost =
      num(inputParams?.scoutingCost) ||
      750;
    const projection = Array.from({ length: 5 }, (_, index) => {
      ...
      const projectedValue = demoProjectionValue(
        currentValue,
        projectedOverall,
        year
      );
      const totalCost =
        acquisitionCost +
        scoutingCost +
        annualDevelopmentCost * year;
      const roiPercent =
        totalCost > 0
          ? ((projectedValue - totalCost) / totalCost) * 100
          : 0;
      return {
        year,
        horizon: 'Year ' + year,
        projectedValue,
        projectedValueFormatted: formatMoney(projectedValue),
        totalCost,
        totalCostFormatted: formatMoney(totalCost),
        roiPercent: Math.round(roiPercent)
      };
    });
    const final = projection[projection.length - 1];
    const suitability =
      final.roiPercent >= 250
        ? 'Strong financial fit'
        : final.roiPercent >= 100
          ? 'Positive financial fit'
          : 'Review required';
    result = {
      type,
      financialGoal:
        inputParams?.financialGoal ||
        'Balanced value growth',
      currentTransferValue: {
        value: currentValue,
        formatted: formatMoney(currentValue)
      ...
      assumptions: {
        acquisitionCost,
        acquisitionCostFormatted: formatMoney(acquisitionCost),
        annualDevelopmentCost,
        annualDevelopmentCostFormatted: formatMoney(annualDevelopmentCost),
        scoutingCost,
        scoutingCostFormatted: formatMoney(scoutingCost)
      },
      projection,
      suitability,
      recommendation:
        'Protect the entry cost and validate the development pathway before committing.',
      confidence,
      risks: [
        {
          label: 'Entry cost',
          level: 'Highest sensitivity',
          note: 'The margin reduces if the acquisition price rises above the supplied assumption.'
        },
        {
        ...
        },
        {
          ...
          level: 'Monitor',
          note: 'New Match Facts can change the rating, value and confidence.'
        }
      ],
      valueDrivers: [
        { label: 'Age-band baseline', value: 'Positive', note: player?.age_group || 'Age-band context' },
        { label: 'Position context', value: 'Positive', note: playerPosition(player) + ' role context' },
        { label: 'Overall and potential', value: 'Strong positive', note: overall + '/100 starting rating' },
        { label: 'Evidence confidence', value: confidence.label, note: confidence.note },
        { label: 'Discipline', value: num(player?.red_cards) ? 'Watch' : 'Positive', note: 'Current card record included' },
        { label: 'Scout setup', value: 'Included', note: 'Current team brief and purchasing context' }
      ],
      summary:
        final.roiPercent >= 100
          ? 'The projected upside clears the modelled cost base.'
          : 'The cost base requires further review against the projected upside.',
      paragraphs: [
        'The financial goal was ' +
          (inputParams?.financialGoal || 'Balanced value growth') +
          '. The player starts at ' +
          formatMoney(currentValue) +
          ' and projects to ' +
          final.projectedValueFormatted +
          ' by year five.',
        'Use this output to support negotiation and due diligence. It is not financial advice.'
      ],
      disclaimer:
        'Demo ROI analysis. This is a deterministic decision-support estimate, not financial advice or a guarantee.'
    };
  } else if (type === 'Match Scenario Prediction') {
    const scenarioKey =
      inputParams?.scenarioKey ||
      inputParams?.scenario ||
      'breaking_low_block';
    const scenario =
      MATCH_SCENARIOS.find((item) => item[0] === scenarioKey) ||
      MATCH_SCENARIOS[0];
    const attributesByScenario = {
      breaking_low_block: ['vision', 'passing', 'dribbling', 'composure', 'crossing'],
      high_press: ['stamina', 'pace', 'defending', 'positioning', 'composure'],
      defending_transition: ['pace', 'stamina', 'defending', 'positioning', 'tackling'],
      protecting_lead: ['defending', 'positioning', 'tackling', 'stamina', 'composure'],
      aerial_game: ['jumping', 'heading', 'strength', 'positioning', 'composure'],
      goalkeeper_command: ['gk_handling', 'gk_positioning', 'gk_communication', 'gk_reflexes', 'jumping']
    };
    const evidenceKeys =
      attributesByScenario[scenarioKey] ||
      attributesByScenario.breaking_low_block;
    const evidence = evidenceKeys.map((key) => ({
      attribute: key,
      label: titleCase(key),
      score: Math.round(demoAttr(player, key)),
      reason: 'Relevant to ' + scenario[1].toLowerCase() + '.'
    }));
    const rawScenarioFit = Math.round(
      average(evidence.map((item) => item.score))
    );
    const scenarioScore = Math.round(
      rawScenarioFit * (0.72 + confidence.score / 360)
    );
    ...
    result = {
      ...
      evidence,
      predictedBehaviour:
        recommendation === 'Flourish'
          ? playerName(player) + ' should create and connect effectively in this repeated tactical demand.'
          : playerName(player) + ' can contribute, but the weaker evidence areas need active tactical support.',
      tacticalNote:
        'Use the result as a selection signal, then confirm the repeated behaviour through live observation.',
      selectionGuidance: [
        {
          title: 'Use the strongest evidence',
          note: 'Build the player’s role around ' +
            evidence.slice().sort((a, b) => b.score - a.score).slice(0, 2).map((item) => item.label).join(' and ') +
            '.'
        },
        {
          title: 'Provide nearby support',
          note: 'Give the player simple passing and cover options around the repeated action.'
        },
        {
          title: 'Protect the lowest score',
          note: 'Do not make ' +
            evidence.slice().sort((a, b) => a.score - b.score)[0].label +
            ' the only route to success.'
        }
      ],
      liveProof: [
        'Decision speed when opposition pressure arrives earlier.',
        'Movement immediately after the first action.',
        'Consistency when the scenario repeats late in the match.'
      ],
      confidence,
      summary:
        playerName(player) +
        ...
        scenarioScore +
        '/100 for ' +
        scenario[1].toLowerCase() +
        '.',
      paragraphs: [
        'The selected match scenario was ' + scenario[1] + '. The result uses the most relevant Coach-rated attributes and current evidence confidence.',
        'Live observation should confirm decision speed, repeatability and behaviour when the opponent removes the player’s preferred option.'
      ],
      disclaimer:
        'Demo match-scenario prediction. This is a deterministic decision-support estimate, not a guarantee.'
    };
  } else {
    const targetPosition =
      String(inputParams?.targetPosition || currentRole.role || '').toUpperCase();
    const target =
      roles.find((role) => String(role.role).toUpperCase() === targetPosition) ||
      {
        role: targetPosition || currentRole.role,
        score: Math.max(45, currentRole.score - 8),
        note: 'Selected target role'
      };
    const gap = Math.max(0, currentRole.score - target.score);
    const verdict =
      gap <= 2
    ...
    const conversions = roles
      .filter((role) => role.role !== currentRole.role)
      .slice(0, 3)
      .map((role) => ({
        ...role,
        reason:
          role.note +
          '. Current football evidence supports a ' +
          role.score +
          '/100 role score.'
      }));
    result = {
      ...
      targetRoleScore: target.score,
      targetVerdict: verdict,
      targetGapVsBest: gap,
      bestCurrentPosition: currentRole.role,
      bestCurrentScore: currentRole.score,
      bestFuturePosition: futureRole.role,
      bestFutureScore: Math.min(99, futureRole.score + 2),
      topRoles: roles,
      roleScores: roles,
      conversionCandidates: conversions,
      nearbyConversionRoles: conversions,
      overallBreakdown: {
        technicalScore: Math.round(average([
          demoAttr(player, 'passing'),
          demoAttr(player, 'dribbling'),
          demoAttr(player, 'shooting'),
          demoAttr(player, 'crossing')
        ])),
        tacticalIQScore: Math.round(average([
          demoAttr(player, 'vision'),
          demoAttr(player, 'positioning'),
          demoAttr(player, 'composure')
        ])),
        physicalProfileScore: Math.round(average([
          demoAttr(player, 'pace'),
          demoAttr(player, 'strength'),
          demoAttr(player, 'stamina'),
          demoAttr(player, 'agility')
        ])),
        mentalCoachabilityScore: Math.round(average([
          demoAttr(player, 'composure'),
          demoAttr(player, 'vision')
        ])),
        ...
        disciplineScore: Math.max(
          40,
          100 - num(player?.yellow_cards) * 5 - num(player?.red_cards) * 20
        ),
        availabilityScore: Math.min(100, 55 + num(player?.appearances) * 5),
      ...
      valueContext: {
        currentValue,
        currentValueFormatted: formatMoney(currentValue),
        budgetLabel: currentValue >= 150000 ? 'Priority budget case' : 'Accessible youth case',
        riskLabel: confidence.score >= 70 ? 'Balanced risk' : 'Evidence risk'
      },
      confidence,
      recommendation:
        gap <= 2
          ? 'Assess the player in the selected role without forcing a positional conversion.'
          : 'Use live observation to confirm whether the conversion gap can be closed.',
      summary:
        gap <= 2
          ? 'Keep ' + playerName(player) + ' in the ' + titleCase(target.role) + ' pathway.'
          : 'The selected role requires a controlled conversion plan.',
      paragraphs: [
        playerName(player) +
          '’s strongest current role is ' +
          currentRole.role +
          ' at ' +
          currentRole.score +
          '/100, with ' +
          futureRole.role +
          ' as the strongest future alternative.',
        'The selected target role is ' +
          target.role +
          '. ScoutLink rates this fit at ' +
          target.score +
          '/100 based on role-weighted attributes, physical suitability, match evidence and age-band context.'
      ],
      disclaimer:
        'Demo Position Fit prediction. This is a deterministic decision-support estimate, not a guarantee.'
    };
  }
  ...
  state.predictions = safeArray(state.predictions);
  state.predictions.unshift(log);
  state.predictionHistory = state.predictions;
  state.demoUsage = {
    predictionsUsed: num(state.demoUsage?.predictionsUsed) + 1,
    exportsUsed: num(state.demoUsage?.exportsUsed),
    interestsUsed: num(
      state.demoUsage?.interestsUsed,
      safeArray(state.pipeline).length
    )
  };
  ...
  const planLimit = 1200;
  const creditsRemaining = Math.max(
    0,
    planLimit - state.demoUsage.predictionsUsed
  );
  log.creditsRemaining = creditsRemaining;
  ...
  return {
    ...
    logId: log.id,
    creditsRemaining,
    predictionsRemaining: creditsRemaining
  };
}
  ...
  async function mountPredictions() {
    ...
    runtime.querySelector(
      ...
      try {
        ...
        const inputParams = predictionInput(predictionType);
        const payload = isDemoExperience()
          ? runDemoPrediction(player, predictionType, inputParams)
          : await request(
              'POST',
              '/api/predictions/run',
              {
        ...
        const log = payload.log || normalisePredictionLog(
          {
            id: payload.logId,
            player_id: playerId,
            player,
  ...
  if (isDemoExperience()) {
    ...
    const allPredictions = [
      ...safeArray(state.predictions),
      ...safeArray(state.predictionHistory),
      ...safeArray(state.prediction_history)
    ];
    const predictions = allPredictions
      .map((log) => normalisePredictionLog(log))
      .filter(
        (log) =>
          String(log.player_id) === String(playerId) &&
          canonicalPredictionType(log) === 'Position Fit Projection'
      )
      .sort(
        (a, b) =>
          new Date(b.run_at || 0) -
    ...
    profileContextCache = {
      ...payload,
      player: payload.player || payload.data || payload,
      workflow:
        safeArray(state.workflowEntries).filter(
          (entry) => String(entry.player_id) === String(playerId)
        ),
      teamScouts: [
        {
          id: 'demo-scout-noah',
  ...
  profileContextCache = {
    ...payload,
    positionFitUnlocked:
      !!payload.positionFitUnlocked ||
      !!payload.latestPositionFit,
    interestRegistered:
      !!payload.pipeline ||
      !!payload.interestRegistered
  };
  return profileContextCache;
}
  ...
  const candidates = all(
    '.role-analysis,.role-card,[data-role-analysis],section,article,.panel'
  ).filter((node) => {
    const text = elementText(node).toLowerCase();
    return (
      text.includes('role analysis') ||
      text.includes('strongest current role') ||
      text.includes('strongest future alternative')
    );
  });

  const wrapper = candidates
    .sort((a, b) => elementText(a).length - elementText(b).length)[0] ||
    findCompactTextElement('strongest current role');

  if (!wrapper) return;

  const protectedNodes = Array.from(
    wrapper.querySelectorAll(
      '.slwf-role-locked,.slwf-role-protected,.blurred-role-output,[data-role-protected]'
    )
  );
  const contentNode =
  ...
  let action = wrapper.querySelector('.slwf-role-lock-action');
  if (!action) {
  ...
  if (context?.positionFitUnlocked && context?.latestPositionFit) {
    [contentNode, ...protectedNodes].forEach((node) => {
    ...
    action.innerHTML =
      '<button class="slwf-btn slwf-btn-secondary slwf-btn-small" type="button">' +
      'View Position Fit analysis</button>';
  ...
  contentNode.classList?.add('slwf-role-locked');
  contentNode.setAttribute?.('aria-hidden', 'true');
  action.innerHTML =
    '<button class="slwf-btn slwf-btn-small" type="button">' +
    'Run Position Fit prediction to reveal this analysis</button>';
  action.querySelector('button').onclick = () => {
  ...
  async function registerInterest(button) {
    ...
    try {
      ...
      button.textContent = 'Interest already registered';
      button.dataset.slwfInterestComplete = '1';
      button.disabled = true;
      button.setAttribute('aria-disabled', 'true');
      profileContextCache = null;
      const refreshedContext = await profileContext(true).catch(() => null);
      applyProfileInterestState(
        refreshedContext || {
          pipeline: payload.pipeline || { player_id: playerId },
...
function applyProfileInterestState(context) {
  if (!context?.pipeline && !context?.interestRegistered) return;

  all('button,a,[role="button"]').forEach((button) => {
    const label = elementText(button).toLowerCase();
    if (
      label === 'register interest' ||
      label === 'interest registered' ||
      label === 'already in pipeline' ||
      label === 'interest already registered'
    ) {
      button.textContent = 'Interest already registered';
      button.dataset.slwfInterestComplete = '1';
      button.setAttribute('aria-disabled', 'true');
  ...
  function mountProfileActions(context) {
    ...
    runtime.innerHTML =
      ...
      (context.pipeline ? 'Interest already registered' : 'Register interest') +
      '</button>' +
  ...
  async function mountProfile() {
    ...
    applyProfileInterestState(context);
  ...
  state.setup = cleanSetup;
  state.scoutSetup = cleanSetup;
  state.scoutPreferences = cleanSetup;
  state.players = safeArray(state.players).map((player) => {
    const compatibilityScore =
      demoCompatibilityFromSetup(player, cleanSetup);
    return {
      ...player,
      compatibilityScore,
      compatibility: {
        ...
        label:
          compatibilityScore >= 80
            ...
            : compatibilityScore >= 65
              ...
              : 'Developing fit',
        explanation:
          'Compatibility recalculated from the saved demo Scout setup.'
      },
      compatibilityUpdatedAt: cleanSetup.updatedAt
    };
  });
  ...
  state.pipeline = safeArray(state.pipeline).map((row) => ({
    ...row,
    player:
      byPlayer[String(row.player_id)] ||
      row.player
  }));
  ...
  window.dispatchEvent(
    new CustomEvent('scoutlink:demo-setup-updated', {
      detail: {
        setup: cleanSetup,
  ...
  const conflicting =
    shadow.getElementById?.('experienceShellV4ScoutStyle') ||
...
function installExactDemoBanner(shadow) {
  if (!isPublicDemo() || !shadow) return;
  ...
  exactCopies(shadow).forEach((copy) => {
    const workspace =
      copy.querySelector('.workspace') ||
      copy.querySelector('.mobile-shell') ||
      copy;
    if (!workspace || workspace.querySelector('[data-slwf-demo-banner]')) {
      return;
    }

    const banner = document.createElement('aside');
    banner.className = 'slwf-demo-banner';
    banner.dataset.slwfDemoBanner = '1';
    banner.setAttribute('aria-label', 'Scout public demo notice');
    banner.innerHTML =
      '<div class="slwf-demo-banner-copy">' +
        '<span>Scout public demo</span>' +
        '<p>Explore fictional sample data. Nothing shown belongs to a real player, team, Coach or Scout.</p>' +
      '</div>' +
      '<div class="slwf-demo-banner-actions">' +
        '<a href="/">Back to homepage</a>' +
        '<a class="primary" href="/register/scout">Request Scout access</a>' +
        '<button type="button">Exit demo</button>' +
      '</div>';

    banner.querySelector('button').onclick = () => {
      if (typeof exitPublicDemo === 'function') {
        exitPublicDemo();
      } else {
    ...
    const topbar =
      workspace.querySelector('.desktop-topbar') ||
      workspace.querySelector('.mobile-topbar') ||
      workspace.firstElementChild;

    if (topbar?.parentNode === workspace) {
      workspace.insertBefore(banner, topbar.nextSibling);
    } else {
  ...
  const predictionType = canonicalPredictionType(type);
  ...
  try {
    ...
    const payload = isDemoExperience()
      ? runDemoPrediction(player, predictionType, inputParams)
      : await request('POST', '/api/predictions/run', {
          playerId,
          predictionType,
          inputParams
        });
    ...
    toast(error.message || 'The prediction could not be completed.', 'error');
  } finally {
    button.disabled = false;
  ...
  const normalised = normalisePredictionLog(log);
  exactCopies(shadow).forEach((copy) => {
    copy.querySelectorAll(
      '.role-card,.role-analysis,[data-role-analysis]'
    ).forEach((card) => {
      card.querySelectorAll(
        '.slwf-role-protected,.slwf-role-locked,.blurred-role-output,[data-role-protected]'
      ).forEach((node) => {
        node.classList.remove(
          'slwf-role-protected',
          'slwf-role-locked',
      ...
      replaceExactButton(
        button,
        () => openPredictionOverlay(normalised),
        'slwfPositionResultBound'
      );
    });
  });
}
  ...
  const context = await profileContext(true).catch(() => ({
    player,
    pipeline: null,
    positionFitUnlocked: false,
    latestPositionFit: null
  }));
  ...
  exactCopies(shadow).forEach((copy) => {
    ...
    Array.from(copy.querySelectorAll('button')).forEach((button) => {
      ...
      if (
        ...
        if (context?.pipeline || context?.interestRegistered) {
          button.textContent = 'Interest already registered';
          button.disabled = true;
          button.setAttribute('aria-disabled', 'true');
          button.dataset.slwfInterestComplete = '1';
          ...
          replaceExactButton(
            button,
            (event) => registerInterest(event.currentTarget),
            'slwfInterestBound'
          );
        }
        ...
        replaceExactButton(button, () => {
          window.location.assign(
            '/scout/compare-players?source=next-action&playerA=' +
            encodeURIComponent(player.id)
          );
        ...
        replaceExactButton(
          button,
          () => chooseExport(player, 'profile', null, null),
          'slwfExportProfileBound'
        );
      } else if (label.includes('team and matches')) {
        replaceExactButton(
          button,
          () => scrollToExactHeading(
            shadow,
            ['match statistics', 'last five match facts', 'upcoming fixtures']
          ),
          'slwfTeamMatchesBound'
        );
      } else if (
        label.includes('watch all videos') ||
        label === 'watch videos'
      ) {
        if (!hasVideo) button.remove();
          ...
          replaceExactButton(
            ...
            () => scrollToExactHeading(
              shadow,
              ['video evidence', 'approved clips']
            ),
            'slwfWatchVideosBound'
          );
        }
        ...
        replaceExactButton(
          button,
          () => window.location.assign(
            '/scout/fixtures?player=' +
            encodeURIComponent(player.id)
          ),
          'slwfPlanFixtureBound'
        );
        ...
        replaceExactButton(
          button,
          () => openWorkflowModal({ playerId: player.id }),
          'slwfWorkflowBound'
        );
      } else if (label === 'run roi and value') {
        replaceExactButton(button, () => {
          const panel = copy.querySelector('.prediction-analysis-panel');
          const roi = Array.from(
            panel?.querySelectorAll('.prediction-type') || []
          ).find((candidate) =>
  ...
  if (isDemoExperience() && typeof getDemoState === 'function') {
    const state = getDemoState();
    payload = {
      preferences:
        state.setup ||
        state.scoutSetup ||
        state.scoutPreferences ||
        {},
      scoutTeam: {}
    };
  } else {
  ...
  exactCopies(shadow).forEach((copy) => {
    ...
    const profilePanel = findExactPanel(
      copy,
      ['scout profile', 'team context']
    );

    if (
      profilePanel &&
      profilePanel.dataset.slwfSetupFieldsReady !== VERSION
    ) {
      profilePanel.dataset.slwfSetupFieldsReady = VERSION;
      const fields = Array.from(
        profilePanel.querySelectorAll('.field')
      );
      ...
      fields.forEach((field) => {
        ...
        if (label.includes('team name')) {
          replaceExactInput(
            control,
            prefs.teamName ||
              payload.scoutTeam?.team_name ||
              elementText(control),
            () => {}
          );
        } else if (label.includes('club or organisation')) {
          replaceExactInput(
            control,
            prefs.clubName ||
              payload.scoutTeam?.club_name ||
              elementText(control),
            () => {}
          );
          ...
          replaceExactControl(
            ...
            prefs.scoutCountry || prefs.country || 'England',
            () => {}
          );
        } else if (label.includes('region')) {
          replaceExactInput(
            control,
            prefs.scoutRegion || elementText(control),
            () => {}
          );
        } else if (label.includes('formation')) {
          replaceExactControl(
            control,
            FORMATIONS,
            prefs.formation ||
        ...
        } else if (label.includes('playing style')) {
          replaceExactControl(
            control,
            PLAYING_STYLES,
            prefs.playingStyle ||
    ...
    Array.from(copy.querySelectorAll('button')).forEach((button) => {
      ...
      if (
        ...
        !label.includes('save scout setup')
      ) {
        return;
      }
      ...
      replaceExactButton(button, async (event) => {
        ...
        const selections = (headingNeedle) => {
          const panel = Array.from(
            source.querySelectorAll('section.panel,article.panel')
          ).find((section) =>
            headingText(
              section.querySelector('.panel-head h3,.panel-head h2')
            ).includes(headingNeedle)
          );
          return Array.from(
            panel?.querySelectorAll(
              '.choice.selected b,.choice[aria-pressed="true"] b,' +
              'input:checked + span,input:checked + b'
            ) || []
          ).map(elementText);
        };
        ...
        const original = elementText(target);
        target.disabled = true;
        target.textContent = 'Saving setup…';
        ...
        try {
          ...
          if (isDemoExperience()) {
            response = await request(
              ...
              message: 'Demo Scout setup saved for this session.'
            }));
            applyDemoSetupCompatibility(setup);
          } else {
            response = await request(
          ...
          target.textContent = original || 'Save changes';
        }
      }, 'slwfSetupSaveBound');
    });
  });
