...
/*
 * Verified live ScoutLink packs.
 *
 * These remain as a read-only fallback catalogue if Stripe catalogue lookup is
 * unavailable. Payment creation still requires a valid live Stripe API key,
 * and paid credits are still applied only by the verified Stripe webhook.
 */
...
function stripeConfigured() {
  /*
   * Prefer a restricted live key (rk_live_) with the minimum permissions
   * required by ScoutLink. A standard sk_live_ key remains supported for
   * compatibility with the existing deployment.
   */
  return /^(?:rk|sk)_live_[A-Za-z0-9]+$/
    .test(stripeApiKey());
}

function paymentSurfaceEnabled() {
  return truthy(
    process.env.STRIPE_PAYMENT_SURFACE_ENABLED
  );
}
...
function stripe() {
  if (!stripeConfigured()) {
    const error = new Error(
      'Live Stripe checkout is not configured. Add the live Stripe API key to the scoutlink-api Vercel project.'
    );
    error.status = 503;
    throw error;
  }
...
function integrationIdentifier() {
  ...
  for (let index = 0; index < bytes.length; index += 1) {
    suffix += letters[bytes[index] % letters.length];
  }

  return `scoutlink_${suffix}`;
}
...
function validateTopUpPrice(price) {
  ...
  const platform =
    text(metadata.platform).toLowerCase();
  const category =
    text(metadata.product_category).toLowerCase();
  const type =
    text(metadata.top_up_type).toLowerCase();
  const quantity =
    integer(metadata.quantity_included);
...
async function listPacks(force = false) {
  ...
  if (
    !force &&
    packCache &&
    now - packCacheAt < 5 * 60 * 1000
  ) {
    return {
      packs: packCache,
      source: packCacheSource
    };
  }
  ...
  try {
    ...
    if (!packs.length) {
      throw new Error(
        'No ScoutLink top-up prices were returned by Stripe.'
      );
    }
    ...
    console.warn(
      '[Scout usage] Stripe catalogue unavailable; using verified live fallback:',
      error.message
    );
...
async function loadContext(userId) {
  ...
  if (!scout) {
    const issue =
      new Error('Scout account not found.');
    issue.status = 404;
    throw issue;
  }
...
async function ensureTeam(context) {
  ...
  const { data: team, error } =
    await supabase
      ...
      .insert({
        team_name:
          teamName.endsWith('Workspace')
            ? teamName
            : `${teamName} Scout Workspace`,
        club_name:
          text(scout.club_name, 240) || null,
        status: 'active',
        subscription_plan:
          text(
...
function canLaunchPayment(context) {
  return Boolean(
    paymentSurfaceEnabled() &&
    stripeConfigured() &&
    !context.scout.is_demo
  );
}
...
async function preparedCheckout(
  ...
  const pack =
    catalogue.packs.find(function (item) {
  ...
  if (!pack) {
    const issue =
      new Error(
        'That ScoutLink top-up pack is not available.'
      );
    issue.status = 400;
    throw issue;
  }
  ...
  if (pack.type === 'ask_radar') {
    const issue =
      new Error(
        'Ask Radar top-ups are unavailable until Ask Radar is enabled.'
      );
    issue.status = 409;
    throw issue;
  }
  ...
  return {
    ...
    plan: currentPlan,
    baseLimit:
      baseLimitFor(context, pack.type),
    effectiveLimitBeforePurchase:
      integer(
        currentLimits[pack.limitKey]
      ),
    stripeReady:
      stripeConfigured(),
    demoAccount:
      Boolean(context.scout.is_demo),
    paymentLaunchAllowed:
      canLaunchPayment(context),
    catalogueSource:
      catalogue.source,
    handoffReady: true
  };
  ...
  try {
    ...
    res.json({
      data: {
        ...
        usage,
        packs:
          catalogue.packs.filter(function (pack) {
            return pack.type !== 'ask_radar';
          }),
        purchases,
        stripeReady:
          stripeConfigured(),
        ...
        note:
          allowed
            ? 'Live Stripe Checkout is enabled. Paid usage is applied only after the signed Stripe webhook confirms payment.'
            : 'The Stripe catalogue is available, but payment launch remains disabled until the live API key, webhook secret and STRIPE_PAYMENT_SURFACE_ENABLED=true are configured.'
      }
    });
  } catch (error) {
    console.error(
...
router.post(
  ...
  async function (req, res) {
    try {
      ...
      const allowed =
        prepared.paymentLaunchAllowed;

      res.json({
        data: {
          ...prepared,
          nextStep:
            prepared.demoAccount
          ...
          message:
            prepared.demoAccount
              ? 'This is a demo Scout account. Live payment launch is disabled.'
              : allowed
                ? 'This pack is validated and ready for live Stripe Checkout.'
                : 'This pack is validated, but live checkout is disabled until Stripe production configuration is complete.'
        }
      });
    } catch (error) {
      console.error(
...
router.post(
  ...
  async function (req, res) {
    try {
      ...
      if (!paymentSurfaceEnabled()) {
        return res.status(503).json({
          error:
            'Live Stripe checkout is disabled. Set STRIPE_PAYMENT_SURFACE_ENABLED=true only after the production API key and webhook secret are configured.',
          prepared
        });
      }
      ...
      if (prepared.demoAccount) {
        return res.status(409).json({
          error:
            'Live Stripe checkout is disabled for demo Scout accounts.',
          prepared
        });
      }
      ...
      if (!prepared.stripeReady) {
        return res.status(503).json({
          error:
            'The pack is ready, but a valid live Stripe API key is not configured in Vercel.',
          prepared
        });
      }
      ...
      const metadata = {
        ...
        price_id:
          pack.priceId,
        plan_at_purchase:
          String(prepared.plan),
        base_limit:
          String(prepared.baseLimit),
        effective_limit_before_purchase:
          String(
      ...
      const session =
        await stripe()
          ...
          .create({
            ...
            cancel_url:
              `${baseUrl}/scout/usage?checkout=cancelled`,
            metadata,
            payment_intent_data: {
              metadata
            }
...
module.exports.listPacks =
  async function listPacksForTests(
    force = false
  ) {
    return (
      await listPacks(force)
    ).packs;
  };

module.exports.validateTopUpPrice =
  validateTopUpPrice;

module.exports.loadContext =
  loadContext;

module.exports.baseLimitFor =
  baseLimitFor;

module.exports.VERIFIED_LIVE_PACKS =
  VERIFIED_LIVE_PACKS;

module.exports.stripeConfigured =
  stripeConfigured;

module.exports.paymentSurfaceEnabled =
  paymentSurfaceEnabled;
