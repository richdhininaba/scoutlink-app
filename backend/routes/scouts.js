...
router.post('/add-scout', requireAuth, requireRole('Scout'), async (req, res) => {
  try {
    ...
    const plan = subscriptionPlan||'Core'; const limits = limitsForPlan(plan); const expires = new Date(Date.now()+7*24*60*60*1000);
    ...
    const cl =
      ...
      '&email=' +
      encodeURIComponent(emailAddr.toLowerCase()) +
      '&type=Scout';
    ...
    res.status(201).json({
      message:'Scout added. Secure Scout setup email sent.',
      scoutId:newScout.id,
      loginCode,
      completeLink:cl,
      emailSent:true,
