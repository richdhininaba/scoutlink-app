...
// Set scout preferences (first login / complete-registration flow)
// Accepts both full preferences AND teamWeaknesses (used in complete-registration step 3)
router.post('/preferences', requireAuth, requireRole('Scout'), async (req, res) => {
    try {
          const {
                  preferredPositions, formationFocus, playingStyle, heightPreference, buildPreference,
                  ageRangeMin, ageRangeMax, otherRequirements, footPreference,
                  teamWeaknesses
          } = req.body;

      // Build prefs object — merge teamWeaknesses in if provided
      const prefs = {
              ...
              ageRangeMin: ageRangeMin || 14,
              ageRangeMax: ageRangeMax || 21,
              otherRequirements: otherRequirements || '',
              footPreference: footPreference || 'any',
      ...
      res.json({ message: 'Preferences saved', preferences: prefs });
          ...
          console.error('[Scout Preferences]', err);
...
router.get('/preferences', requireAuth, requireRole('Scout'), async (req, res) => {
    try {
          const { data } = await supabase
            .from('scouts')
            .select('scout_preferences,preferences_set')
            .eq('id', req.user.id)
            .single();
...
router.get('/pipeline', requireAuth, requireRole('Scout', 'Stratex'), async (req, res) => {
    try {
          ...
          let q = supabase
            ...
            .select(
                      'id,stage,notes,interest_level,created_at,players(id,first_name,last_name,specific_position,primary_position,overall_rating,transfer_value,team_name,age,position_group),scouts(id,first_name,last_name,club_name)',
              { count: 'exact' }
                    );
...
router.post('/add-scout', requireAuth, requireRole('Scout'), async (req, res) => {
    try {
          const { data: me } = await supabase
            .from('scouts')
            .select('id,scout_team_id,club_name,is_super_user')
            .eq('id', req.user.id)
            .single();
      ...
      // Duplicate checks across all user tables
      ...
      // Generate globally unique login code
      ...
      const { data: newScout, error } = await supabase.from('scouts').insert({
              ...
              phone: phone || null,
              club_name: scoutClub || me.club_name || null,
              club_league: scoutLeague || null,
      ...
      const baseUrl = config.brandUrl || 'https://scoutlink.app';
          const completeLink = baseUrl + '/frontend/pages/complete-registration.html?code=' + loginCode +
                  '&email=' + encodeURIComponent(emailAddr.toLowerCase()) + '&type=Scout';
          await emailSvc.sendCompleteSignup({
                  to: emailAddr, firstName, loginCode, accountType: 'Scout', completeLink
          }).catch(e => console.error('[Email]', e.message));

      res.status(201).json({
              message: 'Scout added. Complete-registration email sent.',
              scoutId: newScout.id,
              loginCode,
              completeLink
      });
